...
class LabelPrinter:
    ...
    def print_jpeg(self, handle, mode, cut):
        ...
        self._send_and_expect(
            Print(self._active_job, image_size, mode, cut), PrintAnswer
        )
...
class Answer(RegexReader):
    def __init__(self, data, callback_more_data_needed):
        if data[0:47] != '<?xml version="1.0" encoding="UTF-8"?>\n<status>':
            raise ValueError(
                "Expected a XML status response first: " + repr(data[0:47])
            )
        else:
            self.comment = ""
            ...
            if status_end == -1:
                ...
                if status.code != 0:
                    if status.comment and len(status.comment) > 0:
                        raise ValueError(
                            'The XML status code is not OK: "%s"' % status.comment
                        )
                    else:
                    ...
                    while len(data) < status_end + status.datasize + 10:
                        ...
                        data = data + callback_more_data_needed(
                            long_timeout=False, buffer_size=missing
                        )
                    payload_data = data[
                        status_end + 11 : status_end + status.datasize + 10
                    ]
                    ...
                    if payload_data[0 : len(expected_start)].startswith(expected_start):
                        ...
                        raise ValueError(
                            "Expected the payload starting with the specific XML message."
                        )
...
class Config(Answer):
    ...
    def _process_data(self, data):
        ...
        self.tape_length_initial = self.get_float_XML_value(
            "media_length_initial", data, ""
        )
...
class Print(Question):
    def __init__(self, job_number, image_size, mode, cut):
        ...
        Question.__init__(
            ...
            % (
                self.get_mode(mode)["name"],
                self.get_mode(mode)["speed"],
                self.get_mode(mode)["lpi"],
                image_size,
                cut,
                job_specification,
            ),
