...
def main():
    parser = argparse.ArgumentParser(
        description="Remotely control a VC-500W via TCP/IP.",
        allow_abbrev=False,
        add_help=False,
        prog="labelprinter.sh",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--host",
        default="192.168.0.1",
        help="the VC-500W's hostname or IP address, defaults to %(default)s",
    )
    parser.add_argument(
        "-p",
        "--port",
        type=int,
        default=9100,
        help="the VC-500W's port number, defaults to %(default)s",
    )
    ...
    group.add_argument(
        ...
        action="store",
        metavar="JPEG",
        help="prints a JPEG image out of the VC-500W",
    )
    group.add_argument(
        "--get-status",
        action="store_true",
        help="connects to the VC-500W and returns its status",
    )
    group.add_argument(
        "--release",
        type=str,
        metavar="JOB_ID",
        help="tries to release the printer from an unclean lock earlier on",
    )
    ...
    print_group.add_argument(
        "--print-lock",
        action="store_true",
        help="use the lock/release mechanism for printing (error prone, do not use unless strictly required)",
    )
    print_group.add_argument(
        "--print-mode",
        choices=["vivid", "normal"],
        default="vivid",
        help="sets the print mode for a vivid or normal printing, defaults to %(default)s",
    )
    print_group.add_argument(
        "--print-cut",
        choices=["none", "half", "full"],
        default="full",
        help="sets the cut mode after printing, either not cutting (none), allowing the user to slide to cut (half) up to a complete cut of the label (full), defaults to %(default)s",
    )
    print_group.add_argument(
        "--wait-after-print",
        action="store_true",
        help="wait for the printer to turn idle after printing before returning",
    )
    ...
    status_group.add_argument(
        ...
        "--json",
        action="store_true",
        help="return the status information in JSON format",
    )
...
def get_status_json(printer):
    ...
    device_json = {
        ...
        "wlan_mac": configuration.wlan_mac,
    }
    ...
    if configuration.tape_length_initial and status.tape_length_remaining != -1.0:
        ...
        tape_json = {
            "present": True,
            "type": int(configuration.tape_width * 25.4),
            "total": int(mm_total),
            "remain": int(mm_remain),
        }
    else:
...
def get_status(printer):
    ...
    if configuration.tape_length_initial and status.tape_length_remaining != -1.0:
        ...
        tape_remain = " Remaining tape %s%% (%smm out of %smm)." % (
            int(tape_percent),
            int(mm_remain),
            int(mm_total),
        )
    ...
    print(
        ...
        % (
            status.print_state,
            status.print_job_stage,
            status.print_job_error,
            tape_remain,
        )
    )
...
def print_jpeg(printer, use_lock, mode, cut, jpeg_file, wait_after_print):
    ...
    if status.print_state != "IDLE":
        print(
            "Printer is currently %s (%s, %s). Waiting for it to become idle..."
    ...
    if use_lock:
        lock = printer.lock()
        print(
            'Printer locked with message "%s", started printing job %s...'
            % (lock.comment, lock.job_number)
        )
    ...
    try:
        if use_lock:
            ...
            print(
                "Job status: %s, %s, %s. Sending the print command..."
                % (
                    job_status.print_state,
                    job_status.print_job_stage,
                    job_status.print_job_error,
                )
            )
        ...
        if file_type == "image/jpeg":
            printer.print_jpeg(jpeg_file, mode, cut)
            if wait_after_print:
                printer.wait_to_turn_idle()
            print("PRINT OK")
        else:
...
def submit_to_cups_queue(image_path, args):
    ...
    try:
        ...
        print(
            f"❌ Error submitting to CUPS: {e.stderr.strip() if e.stderr else 'Unknown error'}",
            file=sys.stderr,
        )
        print(
    ...
    try:
        ...
        if args.get_status:
            if args.json:
                ...
                get_status(printer)
        elif args.print_jpeg is not None:
            print_jpeg(
                printer,
                args.print_lock,
                args.print_mode,
