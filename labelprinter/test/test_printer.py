...
class MockConnection(object):
    ...
    def register_response_from_files(
        self, file_message, file_response, answer_description
    ):
        logger.debug(
            "Registering message/response pair in mock from files %s and %s",
            file_message,
            file_response,
        )
    ...
    def register_binary_response(self, file_message, file_response, answer_description):
        logger.debug(
            "Registering message/response pair in mock from files (binary)%s and (ASCII)%s",
            file_message,
            file_response,
        )
    ...
    def send_message(self, message):
        ...
        logger.info(
            "Received message of type %s: %s", type(message).__name__, data.encode()
        )
    ...
    def send_file(self, handle):
        ...
        logger.info(
            "Received binary file to send, totalling %s bytes", len(binary_data)
        )
...
class TestPrinter(unittest.TestCase):
    ...
    def _assert_get_configuration(
        ...
        printer._connection.register_response_from_files(
            message, response, answer_description
        )
    ...
    def test_get_configuration(self):
        ...
        self._assert_get_configuration(
            "getconfig.bin", "getconfig.resp.bin", "configuration XML", 1, 197.0, 1.022
        )
    ...
    def test_get_configuration_no_tape(self):
        ...
        self._assert_get_configuration(
            "getconfig.bin",
            "getconfig2.resp.bin",
            "configuration XML (no tape)",
            "",
            "",
            "",
        )

    def _assert_get_status(
        self,
        message,
        response,
        answer_description,
    ...
    def test_lock(self):
        ...
        self._assert_lock_status(
            "lock.bin", "lock.resp.bin", "lock request XML", "set", "L1807901834", 0
        )

    def _assert_release_status(
        self,
        message,
        response,
        answer_description,
    ...
    def _assert_print(
        ...
        printer._connection.register_response_from_files(
            setup_message, setup_response, setup_description
        )
        printer._connection.register_binary_response(
    ...
    def _assert_print_with_lock(
        ...
        printer._connection.register_response_from_files(
            lock_message, lock_response, lock_description
        )
